...
public class DowngradeWithFutureDates {
    ...
    public void run() throws Exception {
        ...
        String currentTierKey = tierIdToTierKey.get(currentTierId);
        if (currentTierKey == null || !tiers.containsKey(currentTierKey)) {
            System.err.println("❌ Invalid or unmapped tierId: " + currentTierId);
            return;
        }

        if (currentTierId == 1) {
            System.out.printf("🔁 Mapping Tier 1 (Silver) to Tier 2 (Gold)%n");
        }

        int threshold = tiers.get(currentTierKey);
        System.out.printf("📊 Using Tier: %s (Threshold: ₹%d)%n", currentTierKey, threshold);
        ...
        LocalDate joinMonth = createdAt.withDayOfMonth(1); // Start from GP join month
        LocalDate evaluationStartMonth = joinMonth.plusMonths(1); // Skip join month
        LocalDate triggerDate = joinMonth.plusMonths(4).withDayOfMonth(10); // 3 months after join
